...
public OnAccountLoad(playerid, type)
{
	...
	if(type == 0)
	{
		...
		SetPVarInt(playerid, "Jailtime", cache_get_field_content_int(0, "Jailtime"));//Check this
		...
		PlayerInfo[playerid][pFaction] = cache_get_field_content_int(0, "Member");
...
public OnPlayerDataSave(playerid)
{
    ...
    printf("SAVING PLAYER DATA... (%s [ID: %i])", PlayerInfo[playerid][pUsername], playerid);
   	...
   	else
   	{
		...
		mysql_format(handlesql, query, sizeof(query), "UPDATE `accounts` SET `Member`='%d', `Rank`='%d' WHERE `ID`='%i'",
		PlayerInfo[playerid][pFaction], PlayerInfo[playerid][pRank],//GetPVarInt(playerid, "Member"), GetPVarInt(playerid, "Rank"),//PlayerInfo[playerid][pFaction], PlayerInfo[playerid][pRank],
		PlayerInfo[playerid][pID]);
		mysql_pquery(handlesql, query);

		mysql_format(handlesql, query, sizeof(query), "UPDATE accounts SET LockerRights=%d, JobReduce=%d, DonateRank=%d, Fightstyle=%d WHERE ID=%i",
  		GetPVarInt(playerid, "LockerRights"), GetPVarInt(playerid, "JobReduce"), GetPVarInt(playerid, "DonateRank"), GetPVarInt(playerid, "Fightstyle"),
		PlayerInfo[playerid][pID]);
		mysql_pquery(handlesql, query);

		mysql_format(handlesql, query, sizeof(query), "UPDATE accounts SET HouseKey=%d, BizKey=%d, RentKey=%i, JobID=%d WHERE ID=%i",
...
public OnPlayerRegister(playerid, password[])
{
	...
	if(IsPlayerConnected(playerid))
	{
	    if(GetPVarInt(playerid, "AccountExist") == 0)
	    {
			...
			//SendClientMessage(playerid, COLOR_ERROR, "You have reached the registeration!! should insert now...");
			//Interior, World, Tut, 0,0,0
	    	mysql_format(handlesql, query, sizeof(query), "INSERT INTO accounts (Name, ForumName, Pass, Cash, Bank, Model, Age, Sex, PosX, PosY, PosZ, Armour, Health) VALUES ('%s', '%s', '%s', 300, 0, 26, %i, %i, 1642.7285, -2240.5591, 13.4945, 0, 50.0)",
	    		PlayerInfo[playerid][pUsername], forum_name, hashpass, GetPVarInt(playerid, "Age"), GetPVarInt(playerid, "Sex"));
	        mysql_pquery(handlesql, query, "OnPlayerRegistered", "i", playerid);//, "OnPlayerRegistered", "i", playerid
	    ...
	    case 2:
	    {
			...
			if(GetPVarInt(playerid, "Jailed") == 3)
			{
				...
				if(unjail == true)
				{
		    		...
		    		mysql_format(handlesql, query, sizeof(query), "UPDATE accounts SET JailedUntil='%s' WHERE ID=%i",
						PlayerInfo[playerid][pJailedUntil], PlayerInfo[playerid][pID]);
					mysql_pquery(handlesql, query);
				}
			}
